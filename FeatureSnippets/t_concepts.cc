...
template <typename T>
concept bool
    Stringable = requires(T a) { // forces you to implement to_string method
  { a.to_string() }
  ->string;
};

template <typename T> concept bool HasStringFunc = requires(T a) {
  { to_string(a) }
  ->string; // has a to_string function which returns a string
};
...
struct Person {
  ...
  string to_string() {
    ...
    return "Weight: " + std::to_string(height) +
           " Height: " + std::to_string(weight);
  }
};
...
void pretty_print(HasStringFunc a) { cout << to_string(a) << endl; }
...
int main() {
  ...
  std::vector<int> v{34, 23, 34, 56, 78}; // we can make it possible to pretty
                                          // print a vector we just need to
                                          // implement -> string
                                          // to_string(vector<int> a); if we
                                          // want to pretty print
  ...
  pretty_print(l);
  // pretty_print(v);
  // this will result in an error , first you get the old template garbage you
  // are used to
  // but you can ignore that because at the end highlighted in blue you will see
  // main.cpp:41:6: note:   constraints not satisfied
  // void pretty_print(Stringable a){
  // ^~~~~~~~~~~~
  // main.cpp:18:14: note: within ‘template<class T> concept const bool
  // Stringable<T> [with T = std::vector<std::__cxx11::basic_string<char> >]’
